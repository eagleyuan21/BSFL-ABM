...
to move ;; larva procedure
  if pushing? [ larvae-push ]
  avoid-obstacle base-speed
  if (not volume-exclusion? or avoid-larvae) [
    fd base-speed
  ]
end
...
to larvae-push
  ...
  let pushing-set ( turtle-set ( (larvae-on neighbors) with [ patch-ahead base-speed = base-patch ] )  ( other larvae-here ) ) ;; patch-ahead 1 not exactly consistent
  if any? pushing-set [
    let heading-stored heading
    let x-component sum [base-speed * dx ] of pushing-set ;;  base-speed * dx + (xcor - xcor-prev) deleted
    let y-component sum [base-speed * dy ] of pushing-set
    if not (x-component = 0 and y-component = 0) [ set heading atan x-component y-component ]
    let dist (x-component ^ 2 + y-component ^ 2)^(.5)
    avoid-obstacle dist ;; added, not exactly good
    fd dist
    ;; check alternate error here
    set heading heading-stored ]
end

to-report avoid-larvae
  ifelse (count larvae-on (patch-ahead base-speed)) > max-larvae-patch [
    report false ]
  [ report true ]
end
...
330
larvae moving
count larvae with [ xcor - xcor-prev != 0 and ycor - ycor-prev != 0]
17
1
11
