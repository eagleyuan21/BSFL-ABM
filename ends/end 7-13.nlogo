...
to designate-zones
  ask patches [
    if not obstacle? [
      ...
      if any? surroundings [
        let closest max-one-of (surroundings with-min [distance myself]) [abs pxcor] ;; change for multiple dishes in one world
          set near-wall? true ] ] ]
end
...
to set-larva-movement ;; not experimentally detailed yet
...
to go
  ...
  tick
  if ticks = 3600 [ show count-close stop ]
