...
to go
  ask larvi [
    ...
    adjust-for-wall
    if spacing? [ adjust-for-larvi2 ]
    recolor
    if can-move-ahead? [ if energy < 5 [ fd 1 ] ]
  ]
  ;; repeat 5 [ ask larvi [ fd 0.2 ] display ] ] ;; former go
  eat-food ;; put under ask larvi later maybe
  redraw-vectors
  do-plotting
  ;; check-food-gone
...
to-report pseudo-vorticity
  report sum [ abs cos ( heading - towardsxy 0 0 - 90 ) ] of turtles / population
end

to-report pseudo-vorticity-directional
   report sum [ cos ( heading - towardsxy 0 0 - 90 ) ] of turtles / population
end
...
to adjust-for-larvi2
  if not can-move-ahead? [
    ...
    while [(i != 180) and (not can-move-ahead?)] [ ;; i?
      rt ((-1) ^ i) * 15 * i ;; checks -15, 30, -45, 60, ...
      set i (i + 1) ] ]
end
