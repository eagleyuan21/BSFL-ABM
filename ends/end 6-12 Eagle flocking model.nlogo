...
to setup
  clear-all
  ask patches [create-food]
  create-turtles population
  [ set color 59 ;; random shades look nice
...
to go-to-food
 ;; ask turtles [if patch-ahead vision = red [turn-towards (towardsxy 0 0) max-food-turn]]
  let food-seen patches in-cone vision 260 with [food-source = true ]
  if any? food-seen [ if energy < 3
    [turn-towards (towards min-one-of food-seen [distance myself]) max-food-turn ]]
end

to flock  ;; turtle procedure
 if not can-move? 1 [if patch-at dx 0 = nobody [
  set heading (- heading)]
...
to eat-food
  ask turtles [if pcolor = red [set pcolor black
    rt 180
    set color color - 0.65
    set energy energy + 1
    set food-source false]
  ]
end
...
true
0
Rectangle -11221820 true false 105 120 195 180
Circle -11221820 true false 75 90 60
Polygon -2674135 true false 165 180 150 210 150 210 165 180
Polygon -6459832 true false 165 180 150 210 165 195 180 210 165 180
Polygon -6459832 true false 75 105 45 120 75 135
Polygon -6459832 true false 195 120 270 240 195 165
Polygon -13791810 true false 135 150 285 195 210 120 135 120
