...
to designate-zones ;; patch setup
  ask patches with [not (wall? or food?)] [
    set near-wall? false
...
to go
  ...
  tick
  if one-trial? and ticks = 3600 [ stop ]
end
...
to eat ;; larva procedure
  if (is-patch? patch-ahead vision) and [food?] of patch-ahead vision [
    ...
    set count-eat-times (count-eat-times + 1)
  ]
end
...
to go-experiment
  go-trial
  set total-fitness (fitness-function + total-fitness)
  set total-count-food (count-food + total-count-food)
  set total-count-wall (count-wall + total-count-wall)
  set total-count-middle ((ticks - count-food - count-wall) + total-count-middle)
  set total-eat-times (count-eat-times + total-eat-times)
  set exp-so-far (exp-so-far + 1)
  if exp-so-far >= num-experiments [
    stop
  ]
end
@#$#@#$#@
...
1062
326
average eating instances
total-eat-times / exp-so-far
2
1
11
