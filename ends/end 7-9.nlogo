...
to create-boundary
  ask patches [ ;; standard is 30 0 < 8, 0 0 >= 50
    set obstacle? false
    set food? false
    if food-present? [
      if not food-offcenter? [ set food-place 0 ]
      if distancexy food-place 0 < 8 [
        set obstacle? true
        set food? true
        set pcolor red ] ]
...
to designate-zones
  ask patches [
    if not obstacle? and any? patches with [obstacle?] in-radius vision [
      ifelse (zone-split? and [food?] of min-one-of (patches in-radius vision with [obstacle?]) [ distance myself ]) or
      (not zone-split? and not any? (patches in-radius vision with [obstacle? and not food?])) [ ;; causes long setup time
        set near-food? true
        set pcolor red - 3 ]
      [ set near-wall? true
        set pcolor gray - 3 ] ] ]
...
to setup-vectors
  if vectors? [
    ifelse dense? [
      ...
      set vector-radius 3
      ask patches [
        if pxcor mod 3 = 0 and pycor mod 3 = 0 and obstacle? = false [
          sprout-vectors 1 [
            set color green
            set size 1.5 ] ] ] ]
    [ set-default-shape vectors "arrow" ;; 2nd option "vector-compass"
      set vector-radius 5
      ask patches [
        if pxcor mod 5 = 0 and pycor mod 5 = 0 and abs pxcor < 45 and abs pycor < 45 and obstacle? = false [
          sprout-vectors 1 [
            set color white
            set size 3 ] ] ] ] ]
  ask vectors [
...
to adjust-for-wall ;; delete/change later
  if [obstacle?] of patch-ahead 1 = true [
    ifelse turning-method? [
      if [ obstacle? ] of patch-ahead 1 = true [
        if [ obstacle? ] of (patch-at dx 0) = true [ ;; does not work with cornors of food
          set heading (- heading) ]
        if [ obstacle? ] of (patch-at 0 dy) = true [
    ...
    [ let i 0
      ...
      while [ not ( can-turn-r? or can-turn-l?) ] [
        set i (i + 1)
        set can-turn-r? ( not [obstacle?] of patch-right-and-ahead (i * increment) 1 )
        set can-turn-l? ( not [obstacle?] of patch-left-and-ahead (i * increment) 1 ) ]
...
to align-to-wall
  let obstacles ((patches in-cone vision 260) with [ obstacle? = true ])
  if any? obstacles [
...
to fd-w [ dist ] ;; forward-wall
  ...
  while [ (dist-to-go > 0) and (can-move? 1) and ([obstacle?] of patch-ahead 1 = false)] [
    fd 1
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count-close"
...
(count-wall - 2600) ^ 2 + (count-close - 700) ^ 2 + ((3600 - count-wall - count-close) - 300) ^ 2
2
1
11
