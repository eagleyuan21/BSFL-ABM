...
to designate-zones
  ask patches [
    if not ( wall? or food? ) [
      set near-wall? false
      let zone-radius 6
      let surroundings ( patches in-radius zone-radius with [ wall? ] )
      if any? surroundings [
...
to setup-larva
  create-larvae 1 [
    ...
    while [ wall? or food? ] [ setxy random-xcor random-ycor ]
    set-larvae-movement
  ]
end

to set-larvae-movement
  ifelse random-speeds?
...
to go
  ask larvae [
    ;; if eating-time-left > 0 [ set eating-time-left (eating-time-left - 1) stop ]
    wiggle
    eat
    align-towards
    move
...
to eat
  ...
  [ set eating-time-left ( eating-time-left - 1)
    set eating-or-not-value 1 ]
  [ if [food?] of patch-ahead vision [
    set eating-time-left max list 0 ( 180 + random 240 - random 240 ) ]
    set eating-or-not-value 0 ]
end
...
to adjust-for
  if [ wall? or food? ] of patch-ahead base-speed = true [
    let i 0
    let increment 3
    ...
    while [ not ( can-turn-r? or can-turn-l?) ] [
      set i ( i + 1 )
      set can-turn-r? ( not [ wall? or food? ] of patch-right-and-ahead ( i * increment ) base-speed )
      set can-turn-l? ( not [ wall? or food? ] of patch-left-and-ahead ( i * increment ) base-speed )
    ]
...
to fd-w [ dist ]
  ...
  while [ ( dist-to-go >= 1 ) and ( can-move? 1 ) and ( [ wall? or food? ] of patch-ahead 1 = false ) ] [
    fd 1
    set dist-to-go ( dist-to-go - 1 )
  ]
  if ( can-move? dist-to-go ) and ( [ wall? or food? ] of patch-ahead dist-to-go = false ) [
    fd dist-to-go
  ]
  if dist < 0 [
    set heading ( heading - 180 )
...
to-report fitness-function
  ...
  let m-optimal ( 0.111 * ticks )
  let count-middle (ticks - count-close - count-wall)
  report ( standardize count-close f-optimal + standardize count-wall w-optimal + standardize count-middle m-optimal) / 3
end

to-report standardize [ experimental optimal ]
...
"" ""
PENS
"" 1.0 0 -16777216 true "" "ask larvae [plot eating-or-not-value]"
...
“Wiggle” is an element of randomness; the larva’s heading is adjusted by a random amount. This amount is based on a centered distribution characterized by “wiggle-amount”. It only wiggles with a probability “wiggle-often”.

The larvae “eat” in this simulation for a duration determined by a random distribution centered about 180 ticks and with a standard deviation of 240 ticks. This is experimentally determined.

“Align-towards” is the most distinctive element of behavior. Experimental evidence roughly suggests that larva tend to follow boundaries. A larva turns to follow the nearest wall patch in parallel, up to a certain angle “max-align-turn”.

The parameter “vision” affects “align-towards” and “eat”. A larva only ‘sees’ a wall or food patch when it is within a distance of vision. Also, a larva have a peripheral vision of only 130 degrees.
...
The initial settings produce what we believe is the larvaes actual behavior. Varying the parameters creates slightly different behavior.

We use the monitors to judge how accurately the simulation behavior is accurate. Examining videos of 10 larva, a larva eats for 25% of the time, follows the wall for 62% of the time, and moves in the middle for 13% of the time. The number of ticks these percentages translate to are shown in notes, and the actual value for the simulation is in the monitor. “Fitness” combines these three measures into one value that usually ranges from 0 to 1, with lower values being more accurate. .7 is bad, .3 is decent, and .15 is great.
...
The position of the food being near the wall rather than in the center as determined by the “food-place” slider impacts the likelihood of a larva finding the food.
...
What happens to the behavior is the larva can smell?

The biggest unresolved question of this model is what happens when there are many larvae. Experimental evidence suggests the very interesting emergent behavior of a vortex around a piece of food. How do many larvae interact with each other? Are they pushed, do they align with each others’ direction, or both?
...
Ethan Brady and Eagle Yuan created this model under direction of Miguel Fuentes-Cabrera at Oak Ridge National Laboratory in the Center for Nanophase Materials Sciences.

Most of the experimental evidence was found by Olga Shishkov and David Hu at Georgia Tech University, this is their paper ___.
