...
to setup-vectors
  if vectors? [
    ask patches [
      if pxcor mod 9 = 0 and pycor mod 9 = 0 and food-source? = false [
        sprout-vectors 1 [
          set color green
          set size 3 ] ] ] ]
end
...
to go
   ; if patch-ahead 1 food-source? = true [stay-put]
  ask larvi [
    set energy energy - 1
    go-to-food
...
to go-to-food
  ...
  let energy-full 5 ;; was changed
  if any? food-seen and (not can-fill? or energy < energy-full) [
...
to-report pseudo-vorticity
  report sum [ abs cos ( heading - towardsxy 0 0 - 90 ) ] of (turtles with [ xcor != 0 and ycor != 0 ] )  / population
end

to-report pseudo-vorticity-directional
  report sum [ cos ( heading - towardsxy 0 0 - 90 ) ] of (turtles with [ xcor != 0 and ycor != 0 ] ) / population
end
...
to adjust-for-larvi2
  if count larvi-on patch-ahead 1 > 1 [
    if any? neighbors with [ count larvi-here = 0 and food-source? = false ] [
      let i 1
      while [patch-ahead 1 = nobody or count larvi-on patch-ahead 1 > 1 ] [
        rt ((-1) ^ i) * 15 * i ;; checks -5, 10, -15, 20, ...
        set i (i + 1)
...
to redraw-vectors
  if vectors? [
    ask vectors [
      ...
      ifelse any? larvi-near [
        let x-component sum [dx] of larvi-near
        let y-component sum [dy] of larvi-near
