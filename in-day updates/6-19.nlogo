...
larvi-own [
  ...
  nearest-neighbor
  hunger
  action-done ;; temporary variables for determining how often flocking occurs
]
vectors-own [ larvi-near
  x-component-sum
  y-component-sum
]
patches-own [ food-source? ] ;; new variable name
globals [ food-gone? ] ;; temporary
...
to setup-vectors
  set-default-shape vectors "default" ;; 2nd option "vector-compass"
  if vectors? [
    ask patches [
      if pxcor mod 5 = 0 and pycor mod 5 = 0 and abs pxcor < 25 and abs pycor < 25 and food-source? = false [
        sprout-vectors 1 [
          set color green
          set size 2 ] ] ] ]
end
...
to go
  ask larvi [
    ...
    wiggle
    go-to-food
    flock ;; compare to reverse order later
    adjust-for-wall
    if spacing? [ adjust-for-larvi ]
    recolor
    eat-food
    ifelse hunger > 45 [ fd 1 ]
    [if ( count larvi-on neighbors * ( random 9 ) / hunger > ( 1 / 3 ) ) [ move-to (min-one-of (neighbors with [ food-source? = false ]) [count larvi ] )  ] ]
  ]
  redraw-vectors
  do-plotting
  ;; check-food-gone
...
to go-to-food
  ...
  if any? food-seen and (not can-fill? or hunger > hunger-full) [
    turn-towards (towards min-one-of food-seen [distance myself]) max-food-turn ;; not flocking after if else statement
    set action-done action-done + 3 ]
end

to flock ;; now separate adjust-for-wall procedure, also occurs after flocking
  find-flockmates
  if flocking? [
    if any? flockmates
    [ find-nearest-neighbor
      ...
      ifelse distance nearest-neighbor < minimum-separation [
        separate
        set action-done action-done + 2 ]
      [ align
        cohere
        set action-done action-done + 1 ] ] ]
end

to adjust-for-wall
  ifelse turning-by-reflection? [
    if not can-move? 1 [
  ...
  [
    let i 1
    while [patch-ahead vision = nobody ] [
      set action-done action-done + 6
      rt ((-1) ^ i) * 5 * i ;; checks -5, 10, -15, 20, ...
      set i (i + 1) ] ]
end

to recolor
  if coloring? [
    ;if action-done >= 3 and action-done < 6 [ set color yellow ] ;; seeking food
    ;if action-done >= 6 and action-done < 9 [ set color red ] ;; avoiding wall
    ;if action-done >= 9 [ set color green ]
    ;if action-done < 3 [  ;; not the above
    ;  if action-done mod 3 = 1 [ set color blue ] ;; flocking
    ;                                              ;; if action-done mod 3 = 2 [ set color green ] ;; separating
    ;  if action-done mod 3 = 0 [ set color white ] ;; absolutely nothing
    ;]
    ;set action-done 0 ] ;; must move this when inspecting
    ifelse action-done = 4 [
...
to do-plotting
  ...
  set-current-plot-pen "calc-order-parameter"
  plot order-parameter

  set-current-plot "pseudo-vorticity"
  set-current-plot-pen "calc-pseudo-vorticity"
  plot pseudo-vorticity
end

to-report pseudo-vorticity
  report sum [ abs cos ( heading - towardsxy 0 0 - 90 ) ] of turtles with [ not ( xcor = 0 and ycor = 0 )] / population
end

to-report pseudo-vorticity-directional
  report sum [ cos ( heading - towardsxy 0 0 - 90 ) ] of turtles with [ not ( xcor = 0 and ycor = 0 ) ] / population
end

to-report order-parameter
  let x-component sum [ cos ( 90 - heading ) ] of larvi
  let y-component sum [ sin ( 90 - heading ) ] of larvi
  report ( ( x-component ) ^ 2 + ( y-component ) ^ 2 ) ^ 0.5 / population
end

to check-food-gone
  if not food-gone? and not any? patches with [ food-source? = true ] [
    show ( ticks )
    set food-gone? true ]
end
  ...
  if count larvi-here > 1 [
    if any? neighbors with [ count larvi-here = 0 and food-source? = false ] [ ;; add no movement part ifelse
      move-to one-of neighbors with [ count larvi-here = 0 and food-source? = false ]
        set action-done action-done + 9 ] ]
end

to adjust-for-larvi2
  if not can-move-ahead? [
    let i 1
    while [(i != 180) and (not can-move-ahead?)] [ ;; i?
      rt ((-1) ^ i) * 30 * i ;; checks -15, 30, -45, 60, ...
...
to redraw-vectors
  if vectors? [
    ask vectors [
      set larvi-near larvi in-radius 5
      if any? larvi-near [
        set x-component-sum (x-component-sum + sum [dx] of larvi-near)
        ...
        let magnitude (x-component-sum ^ 2 + y-component-sum ^ 2)^(.5)
        set color scale-color green magnitude 0 300 ] ] ]
end
to track-larvae
  ask one-of larvi [
    set color yellow
...
to find-nearest-neighbor ;; larvae procedure
...
to-report average-heading-towards-flockmates  ;; larvae procedure
  ;; "towards myself" gives us the heading from the other larvae
  ;; to me, but we want the heading from me to the other larvae,
  ;; so we add 180
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count patches with [food-source?]"
...
"" ""
PENS
"calc-order-parameter" 1.0 0 -16777216 true "" ""
...
"" ""
PENS
"calc-pseudo-vorticity" 1.0 0 -16777216 true "" ""
"calc-pseudo-vorticity-rotational" 1.0 0 -13791810 true "" "plot pseudo-vorticity-directional"
