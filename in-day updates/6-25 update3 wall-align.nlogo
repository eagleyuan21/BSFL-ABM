...
to setup-larvi
  create-larvi population [
    ...
    while [ food-source? or wall? ] [ setxy random-xcor random-ycor ]
...
to setup-vectors
  if vectors? [
    ifelse dense? [
      ...
      set vector-radius 3
      ask patches [
        if pxcor mod 3 = 0 and pycor mod 3 = 0 and abs pxcor < 45 and abs pycor < 45 and food-source? = false [
          sprout-vectors 1 [
            set color green
            set size 1.5 ] ] ] ]
    [ set-default-shape vectors "arrow" ;; 2nd option "vector-compass"
      set vector-radius 5
      ask patches [
        if pxcor mod 5 = 0 and pycor mod 5 = 0 and abs pxcor < 45 and abs pycor < 45 and food-source? = false [
          sprout-vectors 1 [
            set color white
            set size 3 ] ] ] ] ]
end
...
to go
  ...
  tick
  ;if ticks = 300 [ show-vector-stats stop ]
end
...
to adjust-for-wall ;; delete/change later
  ...
  [
    let i 1
    while [not can-move? 1 or ([ food-source? or wall? ] of patch-ahead 1 = true)] [ ;infinite loop here
      rt ((-1) ^ i) * 5 * i ;; checks -5, 10, -15, 20, ...
      set i (i + 1) ] ]
end

to align-to-wall
  ;show "alligning to wall ... "
  let obstacles ((patches in-cone vision 260) with [ food-source? = true or wall? = true ]) ;; then add wall?
  if any? obstacles [
    ;show "THERE IS AN OBSTACLE:"
    let nearest-obstacle (min-one-of obstacles [ distance myself ])
    ;show word "heading towards nearest-obstacle:  " (towards nearest-obstacle)
    ;show word "heading of larvi:  " heading
    ifelse abs (heading - (towards nearest-obstacle)) < 90 [ ;; could create errors
      turn-away (towards nearest-obstacle) max-wall-align
      ;show "it turned away"
    ]
    [ turn-towards (towards nearest-obstacle) max-wall-align
      ;show "it turned towards"
    ] ]
end

to adjust-for-larvi
  let base-patch patch-here
  let pushing-set ( turtle-set ( (larvi-on neighbors) with [ patch-ahead 1 = base-patch ] )  ( other larvi-here ) )
  if any? pushing-set [
...
to fd-w [ dist ]
  let dist-to-go dist
  while [ (dist-to-go > 0) and (can-move? 1) and ([wall?] of patch-ahead 1 = false) ] [
    fd 1
      ...
      if any? larvi-near [
        ifelse average? [
          if ticks > vector-start [
            set x-component-sum (x-component-sum + sum [xcor - xcor-prev] of larvi-near) ;; with will-move?=true, multiple pushing?
...
974
240
average # turtles on same patch
mean [ count other turtles-here ] of larvi
2
1
11
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot mean [ count other turtles-here ] of larvi"
