...
to create-boundary
  ask patches [
    set boundary? false
    if food? and distancexy 15 -15 < 6 [ ;; 15 -15 good
      set boundary? true
      set pcolor red ]
...
to setup-vectors
  if vectors? [
    ifelse dense? [
      ...
      set vector-radius 3
      ask patches [
        if pxcor mod 3 = 0 and pycor mod 3 = 0 and abs pxcor < 45 and abs pycor < 45 and boundary? = false [
          sprout-vectors 1 [
            set color green
            set size 1.5 ] ] ] ]
...
to adjust-for-wall ;; delete/change later
  ...
  [
    let i 1
    while [not can-move? 1 or ([ boundary? ] of patch-ahead 1 = true)] [ ;infinite loop here
...
to fd-w [ dist ] ;; forward-wall
  let dist-to-go dist
  while [ (dist-to-go > 0) and (can-move? 1) and ([boundary?] of patch-ahead 1 = false)] [
    fd 1
