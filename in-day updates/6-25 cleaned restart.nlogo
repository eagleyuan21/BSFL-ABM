...
to setup-larvi
  create-larvi population [
    ...
    if hide-larvi [ set color (insert-item 3 (extract-rgb yellow) 40) ]
    set size 1.5
...
to setup-vectors
  if vectors? [
    ifelse dense? [
      ...
      set vector-radius 3
      ask patches [
        if pxcor mod 3 = 0 and pycor mod 3 = 0 and abs pxcor < 25 and abs pycor < 25 and food-source? = false [
          sprout-vectors 1 [
            set color green
            set size 1.5 ] ] ] ]
...
to go
  if average? and ticks = 20 [ setup-vectors ]
  ask larvi [
    wiggle
