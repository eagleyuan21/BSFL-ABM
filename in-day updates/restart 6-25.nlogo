...
patches-own [ food-source? ]
globals [ vector-radius ]
...
to setup-larvi
  create-larvi population [
    set color white
    if hide-larvi [ set color (insert-item 3 (extract-rgb white) 0) ]
    set size 1
...
to go
  if average? and ticks = 50 [ setup-vectors ]
  ask larvi [
    flock
    wiggle ;; not precise
    adjust-for-wall
    ...
    fd 1  ;; make will-move? turtle variable later
  ]
  manage-vectors
  ask larvi [ eat-food ]
  do-plotting
      ...
      if any? larvi-near [
        ifelse average? [
          if ticks > 50 [
            ...
            set magnitude (x-component-sum2 ^ 2 + y-component-sum2 ^ 2)^(.5) / larvi-total
            if vector-size? [
              ifelse dense? [
                set color scale-color green magnitude 0 .15 ]
              [ set size min list (magnitude / .1 * 3 + 2) 5
              ]
            ]
          ]
        ]
        [ set x-component-sum (sum [dx] of larvi-near) ;; with will-move?=true, multiple pushing?
          set y-component-sum (sum [dy] of larvi-near)
          set x-component-sum2 (sum [abs dx] of larvi-near)
          set y-component-sum2 (sum [abs dy] of larvi-near)
          ...
          if vector-size? [
            ifelse dense? [
              set color scale-color green magnitude .6 1 ]
            [ set size min list (magnitude / 1 * 3 + 2) 5
            ]
          ]
        ]
...
;to highlight-ring
;  let ring patches with [ (abs pxcor >= 10 and abs pxcor <= 13) and (abs pycor >= 10 and abs pycor <= 13) ]
;  ask turtles-on ring [
;    set color (insert-item 3 (extract-rgb yellow) 255) ]
;  ask turtles-on (patches with [ [ not member? ] of ring ]) [
;    set color (insert-item 3 (extract-rgb yellow) 40) ]
;end
