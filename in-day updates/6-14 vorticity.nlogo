...
to go-to-food
  ...
  if any? food-seen and energy < energy-full [
...
to do-plotting
  ...
  histogram [count flockmates] of turtles
...
to-report order-parameter
  let x-component sum [ cos ( 90 - heading ) ] of turtles
  let y-component sum [ sin ( 90 - heading ) ] of turtles
...
to adjust-for-turtles
  if count turtles-here > 1 [
    if any? neighbors with [ count turtles-here  = 0 and food-source? = false ] [ ;; add no movement part ifelse
      move-to one-of neighbors with [ count turtles-here = 0 and food-source? = false ]
        set action-done action-done + 9 ] ]
end
to adjust-for-turtles2
  if count turtles-on patch-ahead 1 > 1 [
    if any? neighbors with [ count turtles-here = 0 and food-source? = false ] [
      let i 1
      while [patch-ahead 1 = nobody or count turtles-on patch-ahead 1 > 1 ] [
...
"calc-pseudo-vorticity-directional" 1.0 0 -10649926 true "" "plot pseudo-vorticity-directional"
...
1308
314
Blue = Directional Vorticity value\nBlack = Vorticity value
10
0.0
1
