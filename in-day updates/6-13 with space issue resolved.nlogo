...
to setup
  clear-all
  create-food ;; the ask statement is in the procedure
  create-turtles population [
    set color white  ;;set color 55 ;; same color now
    set size 1.5
    set action-done 0
...
to go
  ask turtles [
    go-to-food
    if flocking? [ flock ] ;; compare to reverse order later
    adjust-for-wall
    if coloring? [ recolor ]
  ]
  repeat 5 [ ask turtles [ fd 0.2 ] display ] ;; not fd 1
  eat-food
  adjust-for-turtles
  do-plotting
  tick
...
to go-to-food
  ...
  let energy-full 4
  if any? food-seen and energy <= energy-full [ ;; should add energy clause
...
to adjust-for-turtles
  ask turtles [ if
    ...
    [ ifelse ( any? neighbors with [ count turtles-here  = 0 and food-source? = false ] )
...
to recolor
  ...
  if action-done >= 6 [ set color red ] ;; avoiding wall
