...
if ticks >= 3600 [
    stop
    reset-ticks
    clear-all ] ;; if similuation needs to go past 3600 ticks, delete this line
...
to-report visible [ agentset ] ;; helper method for align-with-obstacle
  report agentset in-cone vision peripheral-vision
end
...
to manage-counts
  ...
  set cfood count-food
  set cwall count-wall
  set cmiddle (abs(ticks + 1 - cfood - cwall))
...
to fitness-function
  ...
  set count-middle (ticks - count-food - count-wall) ;; a slight assumption
  ;report ( standardize count-food f-optimal + standardize count-wall w-optimal + standardize count-middle m-optimal) / 3
set ff ((((count-food - f-optimal) / f-optimal) ^ 2) + (((count-wall - w-optimal) / w-optimal) ^ 2) + (((count-middle - m-optimal) / m-optimal) ^ 2)) / 3
    ;
 ;; [
 ; if ticks >= 1
;  let i (ticks - 1)
;  let column_matrix_sd 1 let row_matrix_sd 3600
;
;  set matrix_sd matrix:make-constant row_matrix_sd column_matrix_sd 0
;  matrix:set matrix_sd  i 0 (ff)
;    set sumff sum matrix:get-row  matrix_sd 0
;    set meanff (sum matrix:get-row  matrix_sd 0) / ticks
...
;;; VIDEO
;; this was in other versions of the code, but it is extraneous for this purpose.
@#$#@#$#@
GRAPHICS-WINDOW
258
28
...
231
344
Other options,\nnot variables we would like to test:
11
0.0
1
...
ask one-of larvi [ if distancexy 30 0 &lt; 5 [ set count-close ( count-close + 1) ] ]</go>
  ...
  <experiment name="Wiggle effect" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3600"/>
    <metric>(ticks - count-food - count-wall)</metric>
    ...
    <enumeratedValueSet variable="food-xcor">
      <value value="22"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="random-speeds?">
      <value value="true"/>
    ...
    <steppedValueSet variable="wiggle-amount" first="0" step="10" last="100"/>
    <steppedValueSet variable="wiggle-often" first="0" step="10" last="100"/>
  </experiment>
  <experiment name="vision" repetitions="3" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3600"/>
    <metric>count-food</metric>
    <metric>count-wall</metric>
    <metric>count-middle</metric>
    <steppedValueSet variable="vision" first="0" step="0.3" last="1.5"/>
  </experiment>
  <experiment name="wiggle amount" repetitions="3" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3600"/>
    <metric>count-food</metric>
    <metric>count-wall</metric>
    <metric>count-middle</metric>
    <steppedValueSet variable="wiggle-amount" first="0" step="10" last="50"/>
  </experiment>
  <experiment name="peripherial" repetitions="3" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3600"/>
    <metric>count-food</metric>
    <metric>count-wall</metric>
    <metric>count-middle</metric>
    <steppedValueSet variable="peripheral-vision" first="200" step="10" last="250"/>
  </experiment>
  <experiment name="max align turn" repetitions="3" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3600"/>
    <metric>count-food</metric>
    <metric>count-wall</metric>
    <metric>count-middle</metric>
    <steppedValueSet variable="max-align-turn" first="0" step="5" last="25"/>
  </experiment>
  <experiment name="wiggle often" repetitions="3" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="3600"/>
    <metric>count-food</metric>
    <metric>count-wall</metric>
    <metric>count-middle</metric>
    <steppedValueSet variable="wiggle-often" first="0" step="2.5" last="12.5"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
