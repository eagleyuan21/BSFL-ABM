...
to setup
  ...
  ask patches [
    set wall? false
    set food? false ]
  let center-coordinates (list [30 30] [90 30] [150 30] [210 30] [270 30] [30 90] [90 90] [150 90] [210 90] [270 90])
  foreach center-coordinates [ pair ->
    set x-center (first pair)
    set y-center (last pair)
    setup-dish
  ]
...
to create-food-wall ;; patch setup
  ask patches [
    if food-present? [
      if distancexy (x-center + food-xcor) y-center < 5 [ ;; circular food source. Arbitrary size
        set food? true
        set pcolor red
      ]
    ]
    if distancexy x-center y-center >= 30 and (abs (pxcor - x-center) <= 30 and abs (pycor - y-center) <= 30) [ ;; circular wall like a petri dish
      set wall? true
      set pcolor gray
    ]
  ]
...
to setup-larva
  create-larvae 1 [
    ...
    setxy (x-center - 30 + random 60) (y-center - 30 + random 60)
    while [ wall? or food? ] [ setxy (x-center - 30 + random 60) (y-center - 30 + random 60) ] ;; prevents spawning in wall or food patches
    set-larvae-movement
]
end
...
"" ""
PENS
"" 1.0 0 -16777216 true "" "ask larva 0 [ ifelse eating-time-left > 0 [ plot 1] [ plot 0]]"
